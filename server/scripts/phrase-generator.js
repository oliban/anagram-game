...
function generateClue(phrase, language = 'en') {
  console.log(`🧩 Legacy clue generation called for "${phrase}" (${language}) - should use AI generation instead`);
...
function createAIPrompts(difficultyRange, count, language, difficultyLevel) {
  ...
  const basePrompt = language === 'sv' 
    ? `Generera ${count} meningsfulla svenska fraser för ordpussel (svårighetsgrad: ${difficultyLevel}). 
       Varje fras ska vara 2-4 ord (max 7 bokstäver per ord) med korrekt svensk grammatik (adjektiv-substantiv överensstämmelse).
       
       För varje fras, skapa också en KLYFTIG ledtråd som:
       - Kräver lateral tänkande, inte bara beskriver vad frasen betyder
       - Använder kulturella referenser, metaforer, eller kreativa associationer
       - Aldrig innehåller ord från själva frasen
       - Är en pusselgåta i sig själv
       
       Exempel på klyftig ledtråd-stil:
       - "kall vinter" → "Snögubbens favoritårstid" 
       - "varm sol" → "Solpanelens bästa vän"
       - "lugn sjö" → "Naturens perfekta spegel"
       
       Returnera exakt denna JSON-format:
       [
         {"phrase": "mjukt regn", "clue": "Paraplyers mjuka påminnelse"},
...
async function generateMeaningfulPhrases(prompt, language, difficultyLevel) {
  console.log(`🤖 Generating phrases AND clever clues with AI: ${prompt.substring(0, 100)}...`);
...
async function useTaskTool(prompt, language, difficultyLevel) {
  ...
  console.log(`🤖 AI generating ${language} phrases AND clever clues...`);
      ...
      try {
        ...
        if (roundedDifficulty >= (minDiff - CONFIG.difficultyTolerance) && 
          ...
          if (generatedPhrases.length % 25 === 0) {
            console.log(`📊 Generated ${generatedPhrases.length}/${targetCount} phrases (${totalProcessed} total processed)`);
          }
          ...
          console.log(`⚠️ "${phrase}" difficulty ${roundedDifficulty} outside target range ${minDiff}-${maxDiff}`);
        }
      } catch (error) {
        console.warn(`⚠️ Error calculating difficulty for "${phrase}": ${error.message}`);
      }
    }
  }
...
async function main() {
  ...
  if (!args.range) {
    console.error('❌ Error: --range parameter is required');
  ...
  if (rangeParts.length !== 2) {
    console.error('❌ Error: Range must be in format "MIN-MAX" (e.g., "0-50")');
    process.exit(1);
  }
  ...
  if (isNaN(minDiff) || isNaN(maxDiff) || minDiff >= maxDiff) {
    console.error('❌ Error: Invalid range. MIN must be less than MAX and both must be numbers');
    process.exit(1);
  }
