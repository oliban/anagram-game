...
module.exports = (dependencies) => {
  ...
  router.get('/api/phrases/for/:playerId', async (req, res) => {
    try {
      ...
      const { calculateScore } = require('../services/difficultyScorer');
  ...
  router.post('/api/phrases/:phraseId/complete', async (req, res) => {
    try {
      ...
      const completionResult = await transaction(async (client) => {
        ...
        console.log(`✅ DATABASE: Completion recorded for phrase ${phraseId}, completion ID: ${completionResult.rows[0]?.id || 'conflict-ignored'}`);
        
        if (completionResult.rows.length === 0) {
          console.warn(`⚠️ COMPLETE: Phrase ${phraseId} already completed by player ${playerId} - skipping duplicate`);
        }
      ...
      // Calculate total score including emoji bonus points
      const totalScore = finalScore + emojiCollectionResult.pointsEarned;
      
      const apiResponse = {
        success: true,
        completion: {
          finalScore: totalScore, // Now includes emoji bonus points
          baseScore: finalScore, // Original phrase score without emoji bonus
          emojiBonus: emojiCollectionResult.pointsEarned,
          hintsUsed: hintsUsed,
      ...
      console.log(`🎉 COMPLETE: Player ${player.name} completed phrase "${phrase.content}" with total score ${totalScore} (base: ${finalScore} + emoji bonus: ${emojiCollectionResult.pointsEarned}, ${hintsUsed} hints, ${completionTime}ms)`);
