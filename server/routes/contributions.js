  ...
  router.post('/api/contribution/:token/submit', async (req, res) => {
    try {
      ...
      try {
        ...
        // Check if it's a validation error and send the specific message
        if (dbError.message && dbError.message.includes('Validation failed:')) {
          // Extract just the validation message part (remove "Validation failed: " prefix)
          const validationMessage = dbError.message.replace('Validation failed: ', '');
          return res.status(400).json({ 
            success: false, 
