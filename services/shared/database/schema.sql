...
-- Function to complete phrase with hints support
...
BEGIN
    ...
    -- Get phrase difficulty for scoring
    SELECT difficulty_level INTO phrase_difficulty FROM phrases WHERE id = phrase_uuid;
    
    -- Calculate score (for now, just use difficulty level * 10, hints will be handled by HintSystem)
    calculated_score := COALESCE(phrase_difficulty, 1) * 10;
    
    -- Mark phrase as completed (ignore conflicts)
    INSERT INTO completed_phrases (player_id, phrase_id, score, completion_time_ms)
    VALUES (player_uuid, phrase_uuid, calculated_score, completion_time_ms)
    ON CONFLICT (player_id, phrase_id) DO NOTHING;
    ...
    -- Return success with calculated score (hints_used = 0 for now, will be enhanced later)
    RETURN QUERY SELECT TRUE::BOOLEAN, calculated_score::INTEGER, 0::INTEGER;
END;
$$ LANGUAGE plpgsql;