...
-- Core business logic functions for scoring, leaderboards, and contribution management
...
-- Function: generate_contribution_token - Token generation
...
-- Player Scores Tracking System
-- Tracks player performance across different time periods

-- Player scores aggregated by period
...
-- Leaderboards for displaying top performers
...
-- Hint Usage Tracking
-- Tracks when players use hints for phrases
...
CREATE INDEX idx_player_scores_player_period ON player_scores(player_id, score_period);
CREATE INDEX idx_player_scores_period_rank ON player_scores(score_period, rank_position);
CREATE INDEX idx_player_scores_period_score ON player_scores(score_period, total_score DESC);

CREATE INDEX idx_leaderboards_period_rank ON leaderboards(score_period, period_start, rank_position);
CREATE INDEX idx_leaderboards_period_score ON leaderboards(score_period, period_start, total_score DESC);
