...
router.post('/phrases/batch-import', async (req, res) => {
  try {
    ...
    // Log admin activity (no broadcasting needed in admin service)
    if (results.totalSuccessful > 0) {
      console.log(`🔧 ADMIN ACTIVITY: Admin batch import: ${results.totalSuccessful} phrases imported by ${adminId}`);
    }
